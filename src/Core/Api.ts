...
export default class Api {
  private request<T> (request: (api: AxiosInstance, resolve: (value: any) => void, reject: (reason?: any) => void) => void): Promise<T> {
    const api = axios.create({
      baseURL: Environment.getEndPoint(),
      headers: {
        'Content-Type': 'application/json',
        Authorization: Environment.getApiKey(),
      }
    })
  ...
  public get<T extends any, U extends any> (path: string, data?: U, options?: Options) {
    return this.client<T, U>('GET', path, data, options)
  }
  ...
  public objectToQuery<T> (query: Record<string, string>): string {
